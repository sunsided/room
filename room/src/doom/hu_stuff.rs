...
/// Create a `*mut c_char` pointer to a NUL-terminated string literal.
///
/// Appends `"\0"` at compile time and casts the resulting `&[u8]` pointer to
/// `*mut c_char`. The pointed-to data lives in the read-only data segment, so
/// the `mut` qualifier exists only to satisfy C-FFI signatures that declare
/// `char *` without `const`; the data must never actually be mutated.
macro_rules! cstr {
    ($s:literal) => {
